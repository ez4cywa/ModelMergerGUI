...
namespace Cast
{
    ...
    public class BlendShape : CastNode
    {
        ...
        public Mesh BaseShape()
        {
            ...
            {
                return (Mesh)ParentNode.ChildByHash((ulong)Value.Values[0]);
            }
            ...
            if (Properties.TryGetValue("vi", out CastProperty Value))
            {
                foreach (var Item in Value.Values)
                {
                    if (Value.Type == "i")
                        ...
                        yield return unchecked((int)(ushort)Item);
                    else if (Value.Type == "b")
                        yield return unchecked((int)(byte)Item);
                }
            }
        }
    ...
    public class Mesh : CastNode
    {
            ...
            if (Properties.TryGetValue("wb", out CastProperty Value))
            {
                foreach (var Item in Value.Values)
                {
                    if (Value.Type == "i")
                        ...
                        yield return unchecked((int)(short)Item);
                    else if (Value.Type == "b")
                        yield return unchecked((int)(byte)Item);
                }
            }
        ...
        public Material Material()
        {
            ...
            {
                return (Material)ParentNode.ChildByHash((ulong)Value.Values[0]);
            }
