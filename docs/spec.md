...
- Arm + weapon assembly is an opt-in tool (off by default, enabled from the Settings menu): the weapon's root bone splices into the arms skeleton as a zero-offset child of the target bone (tag_weapon auto-detected, override via dropdown), weapon bone hierarchy and skin weights are preserved with re-indexed weight buffers and collision-suffixed names, weapon texture paths are rewritten to absolute, and the output defaults to `<weapon_code>_viewhands.cast`. Merge outputs default to `<weapon_code>.cast` and magazine fills to `<weapon_code>_filled.cast`, all derived from the shared weapon-code extractor.
