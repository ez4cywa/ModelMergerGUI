...
- Keep a dedicated, always-visible preview drop area above the scrolling groups. It opens one standalone preview per CAST file and also offers click-to-browse multi-selection. Only this region routes drops to preview; group drops append into that group, while other window areas place each batch in a separate group (reuse an idle empty group or create one). No merge is required between batches. Modal dialogs block drops, and busy groups cannot accept imports. Preserve the per-group 15-part limit.
