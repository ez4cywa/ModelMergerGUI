...
- 可从任一已选槽位打开交互式 3D 部件预览，也可在合并完成后预览最终拼接模型；大于等于 2 MiB 或使用 32 位面索引的 Cast 默认由 Rust 解析和抽样。
...
- 全 Rust 原生桌面版按[完整迁移地图](docs/full-rust-migration.md)分阶段实施；迁移期间保留当前 WPF 版作为兼容基线。
...
脚本从小米官网下载经校验的字体包，只提取程序使用的 Medium 字重；中文标题层级通过字号、颜色和间距区分，避免合成粗体造成观感不一致。下载的本地字体文件会被 Git 忽略。官方 GitHub Release 中的可执行文件已经嵌入字体，普通用户无需运行该脚本。
